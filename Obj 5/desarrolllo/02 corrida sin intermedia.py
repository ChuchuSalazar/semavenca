...
class PLSSEMPredictiveAnalyzer:
        ...
        self.models = {
            'Mah': {
                ...
                'variable_names': {
                    ...
                    'SQ': 'Status Quo',
                    'CS': 'Contagio Social',
                    'PCA': 'Propensión Conductual al Ahorro'
                }
            },
            'Hah': {
    ...
    def clean_data(self, data, group):
        ...
        # Variables requeridas para el modelo (solo main_equation predictors + target)
    ...
    def structural_prediction(self, data, group):
        ...
        # Asegurar que SQ venga observada y sea numérica
        if 'SQ' in X_main.columns:
            X_main['SQ'] = pd.to_numeric(
                X_main['SQ'], errors='coerce').fillna(0)
        else:
            if 'SQ' in data.columns:
                X_main['SQ'] = pd.to_numeric(
                    data['SQ'], errors='coerce').fillna(0)
            else:
                X_main['SQ'] = 0.0
        # Reordenar columnas según lista de predictores
        X_main = X_main[main_eq['predictors']]

        if 'SQ' in data.columns:
            X_main['SQ'] = pd.to_numeric(data['SQ'], errors='coerce').fillna(0)
        else:
            X_main['SQ'] = 0.0  # sq_predicted  # Usar SQ predicho
        ...
        for train_idx, test_idx in rkf.split(data):
            ...
            try:
                ...
                X_main_test = test_data[model['main_equation']
                                        ['predictors']].copy()
                if 'SQ' in test_data.columns:
                    X_main_test['SQ'] = pd.to_numeric(
                        test_data['SQ'], errors='coerce').fillna(0)
                else:
                    X_main_test['SQ'] = 0.0
                ...
                main_predictors = [
                    p for p in model['main_equation']['predictors']]
                main_predictors.extend(
                    [])
            ...
            if var in data.columns:
                ...
                if var_impacts:
                    ...
                    sensitivity_results[var] = {
                        ...
                        'impacts': var_impacts,
                        'variable_names': model['variable_names'].get(var, var)
                    }
        ...
        for group in ['Mah', 'Hah']:
            ...
            try:
                ...
                for i, (var, results) in enumerate(list(sensitivity_results.items())[:3]):
                    print(
                        f"  {i+1}. {results['variable_names']}: {results['average_sensitivity']:.4f}")
    ...
    def plot_observed_vs_predicted(self, group):
        ...
        try:
            ...
            # ax1.scatter(sq_predicted, color=color, alpha=0.6, s=60)

            # Línea de referencia perfecta
            # min_val = min(sq_predicted.min())
            # max_val = max(sq_predicted.max())
    ...
    def plot_monte_carlo_distributions(self, group):
        ...
        try:
            ...
            metrics = ['main_r2',
                       'main_rmse']
            titles = ['R² Ecuación Principal',
                      'RMSE Ecuación Principal']
    ...
    def plot_sensitivity_analysis(self, group):
        ...
        try:
            ...
            var_names = [sensitivity_results[var]['variable_names']
            ...
            if variables:
                ...
                ax3.set_title(f'Curva de Sensibilidad\n{sensitivity_results[most_important_var]["variable_names"]}',
                              fontweight='bold')
                ax3.grid(True, alpha=0.3)
    ...
    def generate_detailed_report(self, group):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for var, sens_data in sensitivity_results.items():
                f.write(f"{var:<15} {sens_data['variable_names'][:24]:<25} "
            ...
            if sensitivity_results:
                ...
                f.write(
                    f"✓ VARIABLE MÁS INFLUYENTE: {sensitivity_results[most_important]['variable_names']}\n")
                f.write(
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for group in self.results.keys():
                ...
                for i, (var, data) in enumerate(list(sensitivity.items())[:3]):
                    f.write(
                        f"  {i+1}. {data['variable_names']} (Sens: {data['average_sensitivity']:.4f})\n")
                f.write("\n")
            ...
            f.write("=" * 80 + "\n")
            f.write("Econ. J. Salazar  / / FIN DEL REPORTE COMPARATIVO\n")
            f.write("=" * 80 + "\n")
